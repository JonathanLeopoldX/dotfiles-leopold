...
function copia() {
    ...
    cp -rv "$1/dotfiles/kitty/kitty.conf" "$HOME/.config/kitty/"
    ...
    sudo usermod --shell /usr/bin/bash root
...
function paquetes() {
    ...
    sudo pacman -S --noconfirm kitty bspwm sxhkd feh polybar picom flameshot rofi sddm lsd bat 
    # tema para sddm
...
#pywal
    git clone https://aur.archlinux.org/python-pywal.git
    cd python-pywal
    makepkg --noconfirm -si
    wal -i ~/.config/wallpaper/autumn.jpg
